...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateToggleIcon(theme) {
    toggleButton.innerHTML = `<i class="fas fa-${theme === 'light' ? 'moon' : 'sun'}"></i>`;
  }
  ...
  if (productTableBody) {
      ...
      .catch(error => {
        ...
        productTableBody.innerHTML = `
          <tr><td colspan="7" class="error-message">
            Error loading products. <button onclick="location.reload()">Retry</button>
          </td></tr>`;
      });
    ...
    function updateProducts() {
      ...
      if (sortedProducts.length === 0) {
        productTableBody.innerHTML = '<tr><td colspan="7" class="no-products">No products found.</td></tr>';
        return;
      }
    ...
    // Event listeners for filters and sort with debouncing
    const debouncedUpdateProducts = debounce(updateProducts, 300);
    brandFilter.addEventListener('change', debouncedUpdateProducts);
    priceFilter.addEventListener('change', debouncedUpdateProducts);
    sortBy.addEventListener('change', debouncedUpdateProducts);
  }
});